...
Motor de simulación del modelo de Kuramoto en Java, archivo único \texttt{motor/KuramotoSim.java} ($\sim 6.4$ kB, bajo el límite de 20 kB del enunciado). Sólo motor: el análisis y la animación se hacen aparte sobre los CSV generados.

\section{Estructura del código}

Una sola clase \texttt{KuramotoSim} con un \texttt{main} y métodos auxiliares estáticos. Sin estado global ni dependencias externas (solo \texttt{java.io} y \texttt{java.util}).
...
\midrule
\texttt{main(args)} & Parsea CLI, inicializa $\omega_i$, $\theta_i(0)$ y $A_{ij}$, abre el CSV, corre el loop principal. \\
\texttt{parseArgs(args)} & Lee flags \texttt{--clave valor} a un \texttt{Map}. \\
\texttt{i(...)} / \texttt{d(...)} & Helpers para enteros y dobles con default. \\
\texttt{buildNeighbors(...)} & Construye la lista de vecinos por nodo (\texttt{int[][]}) según topología. \\
\texttt{deriv(...)} & Evalúa $f(\theta)_i = \omega_i + K \sum_{j\in\text{vec}(i)} \sin(\theta_j-\theta_i)$. \\
\texttt{writeRow(...)} & Calcula $r(t)$ y escribe una fila del CSV. \\
\bottomrule
\end{tabular}
\end{center}
...
# Red aleatoria, K=1, p=0.3, semillas separadas
