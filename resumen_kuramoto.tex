...
\begin{description}[leftmargin=1.1cm,style=nextline]
  ...
  \item[(b) Red aleatoria (Erdős--Rényi)] $A_{ij}=1$ con probabilidad $p\in[0,1]$ (al menos 10 valores). Grado medio $\langle k \rangle = p(N-1)$.
...
\midrule
$N$ & Número de osciladores & $N > 500$ & Fijo. Elegir (p.ej.\ $N{=}500$ o $1000$) y mantener constante. \\
...
$dt$ & Paso de integración & Fijo, a elegir & ``$dt$ fijo e intrínseco de la simulación'' (consigna). \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  \item $p \in [0, 1]$, \textbf{al menos 10 valores}.
  \item Para análisis de $r(p)$: fijar $K=1$.
...
\begin{itemize}
  ...
  \item Para análisis de $r(v)$: fijar $K=1$.
...
\subsection*{Elección de $dt$ y $t_{\text{sim}}$ (calibración empírica)}

Ni la bibliografía (\textit{Neuronal Dynamics}) ni el \texttt{Teorica5a.pdf} ni el enunciado prescriben un valor de $dt$: hay que determinarlo por convergencia. La única escala temporal natural es el período de oscilación $T = 2\pi/\langle\omega\rangle \approx 2\pi \approx 6.28$.

\textbf{Test de convergencia para $dt$:} probamos $dt = 10^{-x}$ con $x\in\{1,2,3,4\}$:
...
\midrule
1 & $10^{-1}=0.1$    & $T/63$    & probablemente demasiado grueso \\
2 & $10^{-2}=0.01$   & $T/628$   & candidato razonable \\
3 & $10^{-3}=0.001$  & $T/6280$  & preciso, más caro \\
4 & $10^{-4}=0.0001$ & $T/62800$ & referencia (``ground truth'') \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Protocolo:}
\begin{enumerate}
  \item Fijar las \emph{mismas semillas} de $\omega_i$ y $\theta_i(0)$ para todas las corridas.
  \item Fijar $K$ (p.ej.\ $K=0.5$, zona de transición) y topología (p.ej.\ completa).
  \item Integrar con cada $dt$ hasta el mismo $t_{\text{sim}}$ y comparar las trazas $r(t)$.
  \item Quedarse con el $dt$ \emph{más grande} tal que la diferencia con el siguiente más chico sea despreciable ($\|r(t;dt) - r(t;dt/10)\|_\infty < \text{tol}$).
\end{enumerate}

\textbf{Elección de $t_{\text{sim}}$:} debe cumplir $t_{\text{sim}} \gg \tau_s(K)$. Como $\tau_s$ es justo lo que queremos medir, calibrar con una corrida piloto: graficar $r(t)$ y exigir que el último $\sim 20\%$ del tiempo sea estacionario. El peor caso (más lento) es $K$ cerca de $K_c$. En la práctica de Kuramoto suele bastar $t_{\text{sim}} \in [50, 500]$.
