public class Integrator {
    ...
    private final int N;
    private final double[] k1, k2, k3, k4, tmp;
    ...
    public void step(double[] theta, double[] omega, int[][] nbr, double K, double dt) {
        Dynamics.deriv(theta, omega, nbr, K, k1);
        for (int i = 0; i < N; i++) tmp[i] = theta[i] + 0.5 * dt * k1[i];
        Dynamics.deriv(tmp, omega, nbr, K, k2);
        for (int i = 0; i < N; i++) tmp[i] = theta[i] + 0.5 * dt * k2[i];
        Dynamics.deriv(tmp, omega, nbr, K, k3);
        for (int i = 0; i < N; i++) tmp[i] = theta[i] + dt * k3[i];
        Dynamics.deriv(tmp, omega, nbr, K, k4);
        for (int i = 0; i < N; i++) {
