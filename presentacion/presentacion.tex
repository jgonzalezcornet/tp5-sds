...
\begin{frame}{Algoritmo implementado}
  ...
  \vspace{0.05cm}
  {\footnotesize El acoplamiento se evalúa precomputando $\sin(\theta_j)$ y $\cos(\theta_j)$ una sola vez por paso; $r(t)$ se calcula únicamente en cada dump.}
\end{frame}
...
\begin{frame}{Observables definidos}
  ...
  \begin{equation*}
    \tau_s = \min \left\{ t : \tilde{r}(t) \ge 0.95\, r_{\infty} \right\}
  \end{equation*}

  {\footnotesize $\tilde{r}$: $r(t)$ suavizada con media m\'ovil; $\tau_s$ es el primer instante en que alcanza el $95\%$ de $r_{\infty}$.}
\end{frame}
...
\begin{frame}{Red completa --- tiempo de sincronizaci\'on $\tau_s(K)$}
  \centering
...
\begin{frame}{Red aleatoria --- mapa $\tau_s(p,K)$}
  \centering
...
\begin{frame}{Red anillo --- mapa $\tau_s(v,K)$}
  \centering
